...
void specialSort(int data[], int n) {
    int len = (n + 1) / 2;
    vector<int> nega;
    vector<int> pos;
    for (int i = 0; i < n; ++i) {
        if (data[i] < 0) {
            nega.push_back(data[i]);
        } else if (data[i] > 0) {
    ...
    if (nega.empty()) {
        ...
        for (int i = n - pos.size(); i < n; ++i) {
            tmp[i] = pos[i - n + pos.size()];
        }
    } else if (pos.empty()) {
        for (int i = n - nega.size(); i < n; ++i) {
            tmp[i] = pos[i - n + nega.size()];
        }
    } else if (pos.size() > len) {
        if (nega.size() > (n - pos.size()) / 2)return;

    } else {
        sort(pos.begin(), pos.end(), greater<int>());
        if (pos.size() > nega.size()) {
            for (int i = 0; i < n; i += 2)tmp[i] = pos[i / 2];
            for (int i = 0; i < n; i += 2)tmp[i] = nega[i / 2];
        } else {
            for (int i = 0; i < n; i += 2)tmp[i] = nega[i / 2];
            for (int i = 1; i < n; i += 2)tmp[i] = pos[i / 2];
        }
    }
